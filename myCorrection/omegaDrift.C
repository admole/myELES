...
namespace fv
{
    defineTypeNameAndDebug(omegaDrift, 0);
...
void Foam::fv::omegaDrift::drift(fvMatrix<scalar>& eqn)
{
    ...
    const volScalarField& omega_les = mesh_.lookupObject<volScalarField>("omega_LES");
    ...
    forAll(cells_, i)
    {
        ...
        scalar drift = ramp * wi * (omega[celli] - omega_les[celli]) / (timescale) * vol[celli];
        source[celli] += drift;
...
void Foam::fv::omegaDrift::addSup
(
    fvMatrix<scalar>& eqn,
    const label fieldi
)
...
void Foam::fv::omegaDrift::correct
        (
...
void Foam::fv::omegaDrift::writeData(Ostream& os) const
{
    dict_.writeEntry(name_, os);
}


bool Foam::fv::omegaDrift::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        ...
        if (!coeffs_.readIfPresent("omegaNames", fieldNames_))
        {
            fieldNames_.resize(1);
            fieldNames_.first() = coeffs_.lookupOrDefault<word>("omega", "omega");
        }
