...
namespace fv
{
    defineTypeNameAndDebug(epsilonDrift, 0);
...
void Foam::fv::epsilonDrift::drift(fvMatrix<scalar>& eqn)
{
    ...
    const volScalarField& epsilon_les = mesh_.lookupObject<volScalarField>("epsilon_LES");
    ...
    forAll(cells_, i)
    {
        ...
        scalar drift = ramp * wi * (epsilon[celli] - epsilon_les[celli]) / (timescale) * vol[celli];
        source[celli] += drift;
...
Foam::fv::epsilonDrift::epsilonDrift
(
    const word& name,
...
void Foam::fv::epsilonDrift::addSup
(
    fvMatrix<scalar>& eqn,
    const label fieldi
)
...
void Foam::fv::epsilonDrift::correct
        (
...
void Foam::fv::epsilonDrift::writeData(Ostream& os) const
{
    dict_.writeEntry(name_, os);
}


bool Foam::fv::epsilonDrift::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        ...
        if (!coeffs_.readIfPresent("epsilonNames", fieldNames_))
        {
            ...
            fieldNames_.first() = coeffs_.lookupOrDefault<word>("epsilon", "epsilon");
        }
