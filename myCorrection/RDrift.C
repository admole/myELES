...
namespace fv
{
    defineTypeNameAndDebug(RDrift, 0);
...
void Foam::fv::RDrift::drift(fvMatrix<symmTensor>& eqn)
{

    // access to velocity equation terms
    const scalarField& vol = mesh_.V();
    const volSymmTensorField& R = eqn.psi();
    // scalarField& diag = eqn.diag();
    symmTensorField& source = eqn.source();
    ...
    forAll(cells_, i)
    {
        ...
        symmTensor drift = ramp * wi * (R[celli] - UP2M_les[celli]) / (timescale) * vol[celli];
        source[celli] += drift;
...
void Foam::fv::RDrift::writeData(Ostream& os) const
{
    dict_.writeEntry(name_, os);
}


bool Foam::fv::RDrift::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        ...
        if (!coeffs_.readIfPresent("RNames", fieldNames_))
        {
            fieldNames_.resize(1);
            fieldNames_.first() = coeffs_.lookupOrDefault<word>("R", "R");
        }
